...
class Assignment
  ...
  def create_user(params)
    ...
    new_user = User.new(username:params[:username],password_digest:params[:password_digest]);
    new_user.save
  end
  ...
  def create_todolist(params)
      ...
      # return an instance of the class with primary key (`id`), and dates (`created_at` and `updated_at`) assigned
    new_to_do_list = TodoList.new(list_name:params[:name],list_due_date:params[:due_date])
    new_to_do_list.save
  end
  ...
  def update_password(id, password_digest)
    ...
    user_to_update = User.find_by(id)
    user_to_update.password_digest = password_digest
    user_to_update.save
  end
  ...
  def update_listname(id, name)
      ...
      # (no return is required)
    to_do_list_to_update = TodoList.find_by(id)
    to_do_list_to_update.list_name = name
